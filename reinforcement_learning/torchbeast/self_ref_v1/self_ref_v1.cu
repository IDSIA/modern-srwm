...
// More general implementation than v0, accepts d_out different of d_in.
// In v1, we allocate num threads st. we cover E*E and loop to get M=3*E.
// with better shared memory usage.
...
__global__ void self_ref_forward_kernel_v1(
    ...
    if (threadIdx.x < M) {
        for (int y_part=0; y_part<3; y_part++) {
            shared_results[threadIdx.x + y_part * E] = 0.f;
            shared_y_insert[threadIdx.x + y_part * E] = 0.f;
            shared_y_q[threadIdx.x + y_part * E] = 0.f;
            shared_y_k[threadIdx.x + y_part * E] = 0.f;
        }
    ...
    if (threadIdx.x < 1) {
        shared_results[3 * E] = 0.f;
        shared_y_insert[3 * E] = 0.f;
        shared_y_q[3 * E] = 0.f;
        shared_y_k[3 * E] = 0.f;
    }
    if (threadIdx.x < 4) {
    ...
    for (int sub=0; sub<subblocks_per_seq; sub++) {
        ...
        if (e < E) {
            for (int y_part=0; y_part<3; y_part++) {
                kv_full_idx = kv_idx + y_part * E * E_block;
                shared_Wy[kv_full_idx] = Wy[n][h][e][m + y_part * E];
            }
            if (m == 0) {  // one column
               shared_Wy[3 * E * E_block + e] = Wy[n][h][e][3 * E]; 
            }
    ...
    for (int t=0; t<t_end; t++) {  // main loop over time in the segment
        ...
        for (int sub=0; sub<subblocks_per_seq; sub++) {
            ...
            if (e < E) {
                for (int y_part=0; y_part<3; y_part++) {
                    int kv_full_idx = kv_idx + y_part * E * E_block;
                    y_main = shared_Wy[kv_full_idx] * shared_x[t*E_block + e];
                    atomicAdd(
                        &shared_results[m + y_part * E],
                        y_main
                    );
                }
                // + 1 part
                if (m == 0) {  // one column
                    kv_final_idx = 3 * E * E_block + e;
                    y_main = shared_Wy[kv_final_idx] * shared_x[t*E_block + e];
                    atomicAdd(
                        &shared_results[3 * E],
                        y_main
                    );
                ...
                if (m < 4) {
                    x_idx = t*E_block + e;
                    beta_main = shared_wb[e + m * E] * shared_x[x_idx];
                    atomicAdd(
                        &shared_b_main[m],
                        beta_main
                    );                    
        ...
        for (int sub=0; sub<subblocks_per_seq; sub++) {
            ...
            if (e < E) {
                ...
                for (int y_part=0; y_part<3; y_part++) {
                    int kv_full_idx = kv_idx + y_part * E * E_block;
                    y_qq = shared_Wy[kv_full_idx] * shared_q_main[e];
                    atomicAdd(
                        &shared_y_q[m + y_part * E],
                        y_qq
                    );
                }
                // + 1 part
                if (m == 0) {  // one column
                    kv_final_idx = e + 3 * E * E_block;
                    y_qq = shared_Wy[kv_final_idx] * shared_q_main[e];
                    atomicAdd(
                        &shared_y_q[3 * E],
                        y_qq
                    );
        ...
        for (int sub=0; sub<subblocks_per_seq; sub++) {
            ...
            if (e < E) {
                ...
                for (int y_part=0; y_part<3; y_part++) {
                    int kv_full_idx = kv_idx + y_part * E * E_block;
                    y_kk = shared_Wy[kv_full_idx] * shared_k_main[e];
                    atomicAdd(
                        &shared_y_k[m + y_part * E],
                        y_kk
                    );
                }
                // + 1 part
                if (m == 0) {
                    kv_final_idx = e + 3 * E * E_block;
                    y_kk = shared_Wy[kv_final_idx] * shared_k_main[e];
                    atomicAdd(
                        &shared_y_k[3 * E],
                        y_kk
                    );
        ...
        if (threadIdx.x < M) {
            ...
            for (int y_part=0; y_part<3; y_part++) {
                int m_idx = m + y_part * E;
                shared_y_insert[m_idx] =
                  shared_b_main[0] * (shared_y_q[m_idx] - shared_y_k[m_idx]);
            }
            shared_q_insert[m] =
        ...
        if (threadIdx.x < 1) {
            shared_y_insert[3*E] =
              shared_b_main[0] * (shared_y_q[3*E] - shared_y_k[3*E]);
        }
        __syncthreads();
        ...
        for (int sub=0; sub<subblocks_per_seq; sub++) {
            ...
            if (e < E) {
                for (int y_part=0; y_part<3; y_part++) {
                    int kv_full_idx = kv_idx + y_part * E * E_block;
                    shared_Wy[kv_full_idx] += shared_k_main[e]
                      * shared_y_insert[m + y_part * E];
                }
                // + 1 part
                if (m == 0) {
                    kv_final_idx = e + 3 * E * E_block;
                    shared_Wy[kv_final_idx] += shared_k_main[e]
                      * shared_y_insert[3 * E];
                }
        ...
        if (threadIdx.x < M) {
            ...
            for (int y_part=0; y_part<3; y_part++) {
                int m_idx = m + y_part * E;
                float r1 = shared_results[m_idx];
                atomicAdd(
                    &result[n][h][l][m_idx],
                    r1
                );
                shared_results[m_idx] = 0.f;
                atomicAdd(
                    &y_diff[n][h][l][m_idx],
                    shared_y_q[m_idx] - shared_y_k[m_idx]
                );
                shared_y_q[m_idx] = 0.f;
                shared_y_k[m_idx] = 0.f;
                shared_y_insert[m_idx] = 0.f;
            }
            atomicAdd(
                &q[n][h][l][m],
                shared_q_main[m]
        ...
        if (threadIdx.x < 1) {
            ...
            int p1_idx = 3 * E;
            float r1 = shared_results[p1_idx];
            atomicAdd(
                &result[n][h][l][p1_idx],
                r1
            );
            shared_results[p1_idx] = 0.f;
            atomicAdd(
                &y_diff[n][h][l][p1_idx],
                shared_y_q[p1_idx] - shared_y_k[p1_idx]
            );
            shared_y_q[p1_idx] = 0.f;
            shared_y_k[p1_idx] = 0.f;
            shared_y_insert[p1_idx] = 0.f;
        }
        __syncthreads();
    }
    __syncthreads();
    ...
    for (int sub=0; sub<subblocks_per_seq; sub++) {
        ...
        if (e < E) {
            for (int y_part=0; y_part<3; y_part++) {
                int kv_full_idx = kv_idx + y_part * E * E_block;
                Wy[n][h][e][m + y_part * E] = shared_Wy[kv_full_idx];
            }
            // + 1 part
            if (m == 0) {
                Wy[n][h][e][3 * E] = shared_Wy[3 * E * E_block + e];
            }

            Wq[n][h][e][m] = shared_Wq[kv_idx];
            Wk[n][h][e][m] = shared_Wk[kv_idx];
            if (m < 4) {
                wb[n][h][e][m] = shared_wb[e + m * E];
            }
        }
    }
    __syncthreads();
...
//    const at::cuda::OptionalCUDAGuard device_guard(device_of(queries));
    ...
    const int E_block = subblocks_per_seq * E_per_subblock;
    const int shared_mem_const = E_block*(5*E + 1) + 4*M + 4 * (5*M + 5) + 4;
    // const int shared_mem_const = E_block * 6 * E + 4 * (5 * M + 4) + 4;
    // const int shared_mem_const = E_block * (3 * M + 4) + 4 * (3 * M + 4) + 4;
...
//    cudaFuncSetAttribute(self_ref_forward_kernel_v1,
...
// Backward kernel
__global__ void self_ref_backward_kernel_v1(
    const float_accessor x,
    ...
    // Load the shared memory for KV
    const int shared_kv_size_y = E_block * (3 * M + 1);
    const int shared_kv_size_qkb = E_block * M;
    extern __shared__ float shared_mem[];
    ...
    float* shared_grad_Wq = shared_grad_Wy + shared_kv_size_y;
    float* shared_grad_Wk = shared_grad_Wq + shared_kv_size_qkb;
    float* shared_grad_wb = shared_grad_Wk + shared_kv_size_qkb;
    ...
    float* shared_grad_qq = shared_grad_yq + 3*M + 1;
    ...
    float* shared_gradout_0 = shared_grad_x + M;
    float* shared_gradout_1 = shared_gradout_0 + M*T;
    float* shared_gradout_2 = shared_gradout_1 + M*T;
    float* shared_gradout_3 = shared_gradout_2 + M*T;  // +1 part

    float* shared_x = shared_gradout_3 + T;

    // float* shared_gradout = shared_grad_x + M;
    // float* shared_x = shared_gradout + 3*M*T;
    float* shared_q = shared_x + E_block*T;
    float* shared_k = shared_q + M*T;
    float* shared_beta = shared_k + M*T;

    float* shared_ydiff_0 = shared_beta + 4*T;
    float* shared_ydiff_1 = shared_ydiff_0 + M*T;
    float* shared_ydiff_2 = shared_ydiff_1 + M*T;
    float* shared_ydiff_3 = shared_ydiff_2 + M*T;

    float* shared_qdiff = shared_ydiff_3 + T;

    // float* shared_ydiff = shared_beta + 4*T;
    // float* shared_qdiff = shared_ydiff + 3*M*T;
    float* shared_kdiff = shared_qdiff + M*T;
    float* shared_bdiff = shared_kdiff + M*T;

    if (threadIdx.x < M) {
        for (int y_part=0; y_part<3; y_part++) {
            shared_grad_yq[m + y_part * E] = 0.f;
        }
        shared_grad_x[m] = 0.f;
        shared_grad_q[m] = 0.f;
        shared_grad_k[m] = 0.f;
    ...
    for (int i = threadIdx.x; i < (t_end*M); i += blockDim.x)
    {
        ...
        shared_gradout_0[i] = grad_out[n][h][t_bw][d];
        shared_gradout_1[i] = grad_out[n][h][t_bw][d + E];
        shared_gradout_2[i] = grad_out[n][h][t_bw][d + 2 * E];

        shared_ydiff_0[i] = y_diff[n][h][t_bw][d];
        shared_ydiff_1[i] = y_diff[n][h][t_bw][d + E];
        shared_ydiff_2[i] = y_diff[n][h][t_bw][d + 2 * E];
    }
    // +1 part
    for (int i = threadIdx.x; i < t_end; i += blockDim.x)
    {
        int t = i + l_offset;
        int t_bw = L - 1 - t;
        shared_gradout_3[i] = grad_out[n][h][t_bw][3 * E];
        shared_ydiff_3[i] = y_diff[n][h][t_bw][3 * E];
    }
    ...
    for (int sub=0; sub<subblocks_per_seq; sub++) {
        ...
        if (e < E) {
            for (int y_part=0; y_part<3; y_part++) {
                int kv_full_idx = kv_idx + y_part * E * E_block;
                shared_Wy[kv_full_idx] = Wy[n][h][e][m + y_part * E];
                shared_grad_Wy[kv_full_idx] = grad_Wy[n][h][e][m + y_part * E];
            }
            if (m == 0) {  // one column
                kv_final_idx = e + 3 * E * E_block;
                shared_Wy[kv_final_idx] = Wy[n][h][e][3 * E];
                shared_grad_Wy[kv_final_idx] = grad_Wy[n][h][e][3 * E];
            }
    ...
    for (int t=0; t<t_end; t++) {
        ...
        for (int sub=0; sub<subblocks_per_seq; sub++) {
            ...
            if (e < E) {
                // int kv_full_idx = kv_idx + y_part * E * E_block;
                int kv_full_idx = kv_idx;
                y_ins = shared_ydiff_0[m_abs] * shared_beta[t*4];
                shared_Wy[kv_full_idx] -= shared_k[e_abs] * y_ins;

                kv_full_idx = kv_idx + E * E_block;
                y_ins = shared_ydiff_1[m_abs] * shared_beta[t*4];
                shared_Wy[kv_full_idx] -= shared_k[e_abs] * y_ins;

                kv_full_idx = kv_idx + 2 * E * E_block;
                y_ins = shared_ydiff_2[m_abs] * shared_beta[t*4];
                shared_Wy[kv_full_idx] -= shared_k[e_abs] * y_ins;

                if (m == 0) {
                    kv_final_idx = 3 * E * E_block + e;
                    y_ins = shared_ydiff_3[t] * shared_beta[t*4];
                    shared_Wy[kv_final_idx] -= shared_k[e_abs] * y_ins;
                }
            }
        }
        __syncthreads();
        ...
        for (int sub=0; sub<subblocks_per_seq; sub++) {
            ...
            if (e < E) {
                for (int y_part=0; y_part<3; y_part++) {
                    int kv_full_idx = kv_idx + y_part * E * E_block;
                    // grad v
                    float res = shared_k[e_abs] * shared_grad_Wy[kv_full_idx] 
                      * shared_beta[t*4];
                    atomicAdd(
                        &shared_grad_yq[m + y_part * E],  // Define
                        res
                    );  // grad_v done.
                }
                // grad beta, with sigmoid
                y_diff = shared_ydiff_0[m_abs];
                int kv_full_idx = kv_idx;
                float grad_b = shared_grad_Wy[kv_full_idx] * shared_k[e_abs]
                ...
                y_ins = y_diff * shared_beta[t*4];
                float res_k = shared_grad_Wy[kv_full_idx] * y_ins;
                atomicAdd(
                    &shared_grad_k[e],
                    res_k
                );

                y_diff = shared_ydiff_1[m_abs];
                kv_full_idx = kv_idx + E * E_block;
                grad_b = shared_grad_Wy[kv_full_idx] * shared_k[e_abs]
                    * y_diff * shared_beta[t*4] * (1.f - shared_beta[t*4]);
                // beta[0] for y
                atomicAdd(
                    &shared_grad_beta[0],
                ...
                y_ins = y_diff * shared_beta[t*4];
                res_k = shared_grad_Wy[kv_full_idx] * y_ins;
                atomicAdd(
                    &shared_grad_k[e],
                    res_k
                );

                y_diff = shared_ydiff_2[m_abs];
                kv_full_idx = kv_idx + 2 * E * E_block;
                grad_b = shared_grad_Wy[kv_full_idx] * shared_k[e_abs]
                    * y_diff * shared_beta[t*4] * (1.f - shared_beta[t*4]);
                // beta[0] for y
                ...
                if (m == 0) {
                    kv_final_idx = e + 3 * E * E_block;
                    float res = shared_k[e_abs] * shared_grad_Wy[kv_final_idx] 
                        * shared_beta[t*4];
                    atomicAdd(
                        &shared_grad_yq[3 * E],  // Define
                        res
                    );  // grad_v done.

                    y_diff = shared_ydiff_3[t];
                    grad_b = shared_grad_Wy[kv_final_idx] * shared_k[e_abs]
                        * y_diff * shared_beta[t*4] * (1.f - shared_beta[t*4]);
                    // beta[0] for y
                    atomicAdd(
                        &shared_grad_beta[0],
                    ...
                    y_ins = y_diff * shared_beta[t*4];
                    res_k = shared_grad_Wy[kv_final_idx] * y_ins;
                    atomicAdd(
                        &shared_grad_k[e],
                        res_k
                    );
        ...
        for (int sub=0; sub<subblocks_per_seq; sub++) {
            ...
            if (e < E) {
                for (int y_part=0; y_part<3; y_part++) {
                    int kv_full_idx = kv_idx + y_part * E * E_block;
                    shared_grad_Wy[kv_full_idx] +=
                      shared_grad_yq[m + y_part * E]
                        * (shared_q[e_abs] - shared_k[e_abs]);
                }
                if (m == 0) {
                    kv_final_idx = e + 3 * E * E_block;
                    shared_grad_Wy[kv_final_idx] += shared_grad_yq[3 * E]
                      * (shared_q[e_abs] - shared_k[e_abs]);                
                }
            }
        }
        ...
        for (int sub=0; sub<subblocks_per_seq; sub++) {
            ...
            if (e < E) {
                for (int y_part=0; y_part<3; y_part++) {
                    int kv_full_idx = kv_idx + y_part * E * E_block;
                    float grad_q =
                      shared_grad_yq[m + y_part * E] * shared_Wy[kv_full_idx];
                    atomicAdd(
                        &shared_grad_q[e],
                        grad_q
                    );  // from y_q to q
                ...
                if (m == 0) {
                    kv_final_idx = e + 3 * E * E_block;
                    float grad_q =
                      shared_grad_yq[3 * E] * shared_Wy[kv_final_idx];
                    atomicAdd(
                        &shared_grad_q[e],
                        grad_q
                    );  // from y_q to q
        ...
        for (int sub=0; sub<subblocks_per_seq; sub++) {
            ...
            if (e < E  && m < 4) {
                // grad v
                float res = shared_k[e_abs]
                  * shared_grad_wb[e + m * E] * beta_b;
                atomicAdd(
        ...
        for (int sub=0; sub<subblocks_per_seq; sub++) {
            ...
            if (e < E) {
                int kv_full_idx = kv_idx;
                float grad_x_y =
                    shared_gradout_0[m_abs]
                    * shared_Wy[kv_full_idx];
                atomicAdd(
                    &shared_grad_x[e],
                    grad_x_y
                );
                ...
                kv_full_idx = kv_idx + 2 * E * E_block;
                grad_x_y = shared_gradout_2[m_abs] * shared_Wy[kv_full_idx];
                atomicAdd(
                    &shared_grad_x[e],
                    grad_x_y
                );
                if (m == 0) {
                    int kv_final_idx = e + 3 * E * E_block;
                    grad_x_y = shared_gradout_3[t] * shared_Wy[kv_final_idx];
                    atomicAdd(
                        &shared_grad_x[e],
                        grad_x_y
                    );
        ...
        for (int sub=0; sub<subblocks_per_seq; sub++) {
            ...
            if (e < E) {
                int kv_full_idx = kv_idx;
                shared_grad_Wy[kv_full_idx] +=
                    shared_gradout_0[m_abs] * shared_x[e_abs];

                kv_full_idx = kv_idx + E * E_block;
                shared_grad_Wy[kv_full_idx] +=
                    shared_gradout_1[m_abs] * shared_x[e_abs];

                kv_full_idx = kv_idx + 2 * E * E_block;
                shared_grad_Wy[kv_full_idx] +=
                    shared_gradout_2[m_abs] * shared_x[e_abs];

                if (m == 0) {
                    kv_final_idx = e + 3 * E * E_block;
                    // atomicAdd(
                    //     &shared_grad_Wy[kv_final_idx],
                    //     shared_gradout_3[t] * shared_x[e_abs]
                    // );
                    shared_grad_Wy[kv_final_idx] +=
                        shared_gradout_3[t] * shared_x[e_abs];           
                }
...
//    const at::cuda::OptionalCUDAGuard device_guard(device_of(grad_queries));
    ...
    // int shared_mem_const = (12 * E_block + 5 + 1 + 2)*M + 9;
    // int shared_mem_const = (8 * E_block + 16)*M + 9;
    int shared_mem_const = 2*E_block + 10*M*E_block + 16*M + 10;

    // int shared_mem_per_time = 2 * (5*M + 4) + E_block;
    int shared_mem_per_time = 2 * (5*M + 5) + E_block;
...
//        cudaDevAttrMaxSharedMemoryPerBlockOptin, device_id);
    ...
    int maxF = maxB / sizeof(float);
    // Following is needed for sm > 48KB
//    cudaFuncSetAttribute(self_ref_backward_kernel_v1,
//      cudaFuncAttributeMaxDynamicSharedMemorySize, maxB);
    // cudaDeviceProp prop;
    // cudaGetDeviceProperties(&prop, 0);
    assert(maxF - shared_mem_const > 0 &&
